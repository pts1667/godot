...
struct QuatCompressed48 {
	...
	[[nodiscard]] Quat decompress() const {
		const float fx = static_cast<float>(static_cast<int>(this->x) - 32768) * (1.f / 32768.f);
		const float fy = static_cast<float>(static_cast<int>(this->y) - 32768) * (1.f / 32768.f);
		const float fz = static_cast<float>(static_cast<int>(this->z) - 16384) * (1.f / 16384.f);

		// Recalculate w from the constraint that x^2 + y^2 + z^2 + w^2 = 1
		const float fw_sq = 1.f - fx * fx - fy * fy - fz * fz;
		float fw = std::sqrt(fw_sq > 0.f ? fw_sq : 0.f);
...
static_assert(sizeof(QuatCompressed48) == 6);
...
struct QuatCompressed64 {
	...
	[[nodiscard]] Quat decompress() const {
		const double fx = static_cast<double>(static_cast<int>(this->x) - 1048576) * (1.0 / 1048576.5);
		const double fy = static_cast<double>(static_cast<int>(this->y) - 1048576) * (1.0 / 1048576.5);
		const double fz = static_cast<double>(static_cast<int>(this->z) - 1048576) * (1.0 / 1048576.5);

		// Recalculate w from the constraint that x^2 + y^2 + z^2 + w^2 = 1
		const double fw_sq = 1.0 - fx * fx - fy * fy - fz * fz;
		double fw = std::sqrt(fw_sq > 0.0 ? fw_sq : 0.0);
...
static_assert(sizeof(QuatCompressed64) == 8);
