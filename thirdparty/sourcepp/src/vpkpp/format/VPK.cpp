...
std::optional<std::vector<std::byte>> VPK::readEntry(const std::string& path_) const {
	...
	if (entryLength != entry->extraData.size()) {
		if (entry->archiveIndex != VPK_DIR_INDEX) {
			...
			FileStream stream{this->getTruncatedFilepath() + '_' + string::padNumber(entry->archiveIndex, 3) + std::string{VPK_EXTENSION}};
			if (!stream) {
				return std::nullopt;
			}
...
bool VPK::bake(const std::string& outputDir_, BakeOptions options, const EntryCallback& callback) {
	...
	const auto getArchiveFilename = [this](const std::string& filename_, uint32_t archiveIndex) {
		std::string out{filename_ + '_' + string::padNumber(archiveIndex, 3) + std::string{VPK_EXTENSION}};
			...
			for (auto& [path, entry] : tempEntries) {
				...
				if (entry->unbaked) {
					...
					if (entry->length == entry->extraData.size() && !this->hasCompression()) {
						...
						auto archiveFilename = getArchiveFilename(::removeVPKAndOrDirSuffix(outputPath), entry->archiveIndex);
	...
	if (this->hasExtendedHeader()) {
		...
		// Calculate Footer2
		mbedtls_md5_context wholeFileChecksumMD5;
		mbedtls_md5_init(&wholeFileChecksumMD5);
		mbedtls_md5_starts(&wholeFileChecksumMD5);
		{
			// Only the tree is updated in the file right now
			mbedtls_md5_update(&wholeFileChecksumMD5, reinterpret_cast<const unsigned char*>(&this->header1), sizeof(this->header1));
			mbedtls_md5_update(&wholeFileChecksumMD5, reinterpret_cast<const unsigned char*>(&this->header2), sizeof(this->header2));
		}
		{
			...
			if (this->header1.treeSize > 0) {
				...
				mbedtls_md5_update(&wholeFileChecksumMD5, reinterpret_cast<const unsigned char*>(treeData.data()), treeData.size());
		...
		if (!dirVPKEntryData.empty()) {
			mbedtls_md5_update(&wholeFileChecksumMD5, reinterpret_cast<const unsigned char*>(dirVPKEntryData.data()), dirVPKEntryData.size());
		}
		{
			if (!this->md5Entries.empty()) {
				mbedtls_md5_update(&wholeFileChecksumMD5, reinterpret_cast<const unsigned char*>(this->md5Entries.data()), this->md5Entries.size() * sizeof(MD5Entry));
		...
		if (!this->footer2.treeChecksum.empty()) {
			mbedtls_md5_update(&wholeFileChecksumMD5, reinterpret_cast<const unsigned char*>(this->footer2.treeChecksum.data()), this->footer2.treeChecksum.size());
		}
		if (!this->footer2.md5EntriesChecksum.empty()) {
			mbedtls_md5_update(&wholeFileChecksumMD5, reinterpret_cast<const unsigned char*>(this->footer2.md5EntriesChecksum.data()), this->footer2.md5EntriesChecksum.size());
		}
		mbedtls_md5_finish(&wholeFileChecksumMD5, reinterpret_cast<unsigned char*>(this->footer2.wholeFileChecksum.data()));
		mbedtls_md5_free(&wholeFileChecksumMD5);
...
bool VPK::sign(const std::string& filename_) {
	...
	return this->sign(string::decodeHex(privateKeyHex), string::decodeHex(publicKeyHex));
}
...
void VPK::setVersion(uint32_t version) {
	if ((version != 0 && version != 1 && version != 2 && version != 54) || version == this->header1.version) {
		return;
	}
