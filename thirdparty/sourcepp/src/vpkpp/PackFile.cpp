...
std::size_t erase_prefix(EntryMap &p_map, std::string_view p_prefix) {
	auto iterator = p_map.lower_bound(std::string(p_prefix));
	std::size_t count = 0;
	while (iterator != p_map.end() && std::string_view(iterator->first).starts_with(p_prefix)) {
		iterator = p_map.erase(iterator);
		++count;
	}
	return count;
}

template <typename EntryMap, typename Callable>
void for_each_prefix(EntryMap &p_map, std::string_view p_prefix, bool p_recursive, Callable p_callable) {
	for (auto iterator = p_map.lower_bound(std::string(p_prefix)); iterator != p_map.end(); ++iterator) {
		const std::string_view key_view{iterator->first};
		if (!key_view.starts_with(p_prefix)) {
			break;
		}
		if (!p_recursive) {
			auto relative = key_view.substr(p_prefix.length());
			if (std::ranges::find(relative, '/') != relative.end()) {
				continue;
			}
		}
		p_callable(iterator);
...
std::size_t PackFile::removeDirectory(const std::string& dirName_) {
	...
	std::size_t count = ::erase_prefix(this->entries, dirName);
	count += ::erase_prefix(this->unbakedEntries, dirName);
	return count;
}
...
void PackFile::runForAllEntries(const std::string& parentDir, const EntryCallback& operation, bool recursive, bool includeUnbaked) const {
	...
	::for_each_prefix(this->entries, dir, recursive, [&operation](const auto &entry) {
		operation(entry->first, entry->second);
	});
	if (includeUnbaked) {
		::for_each_prefix(this->unbakedEntries, dir, recursive, [&operation](const auto &entry) {
			operation(entry->first, entry->second);
		});
	}
}
...
void PackFile::runForAllEntriesInternal(const std::string& parentDir, const std::function<void(const std::string&, Entry&)>& operation, bool recursive, bool includeUnbaked) {
	...
	::for_each_prefix(this->entries, dir, recursive, [&operation](auto &entry) {
		operation(entry->first, entry->second);
	});
	if (includeUnbaked) {
		::for_each_prefix(this->unbakedEntries, dir, recursive, [&operation](auto &entry) {
			operation(entry->first, entry->second);
		});
	}
}
...
void PackFile::mergeUnbakedEntries() {
	for (auto entry = this->unbakedEntries.begin(); entry != this->unbakedEntries.end(); ++entry) {
		entry->second.unbaked = false;

		// Clear any data that might be stored in it
		entry->second.unbakedUsingByteBuffer = false;
		entry->second.unbakedData = "";

		this->entries.insert_or_assign(entry->first, entry->second);
	}
	this->unbakedEntries.clear();
}
